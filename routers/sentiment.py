...
def sentiment_by_name(query: str = Query(..., description="Place name or location to analyze")):
    ...
    try:
        ...
        if not search_results:
            raise HTTPException(
                status_code=404,
                detail=f"No places found for '{query}'.",
            )
        ...
        for place in places[:5]:
            ...
            if reviews:
                # Get sentiment insights (includes photo_url from cache/service)
                sentiment = get_sentiment_insights(pid, place_name=place.get("name"))

                # Get one photo URL (for the place block) – uses Places Photos API
                photo_url = sentiment.get("photo_url") or get_place_photo_url(pid)
...
def sentiment_by_id(place_id: str):
    ...
    Returns the same structure as /sentiment/analyze:
    {
      "place": { ... },
      "sentiment": { ... }
    }
    """
    try:
        # Get sentiment (includes place_name + photo_url from cache/service)
        sentiment = get_sentiment_insights(place_id)

        # Fetch basic place details for full place block (Option B)
        details = get_place_basic_details(place_id)

        place_block = {
            "place_id": place_id,
            "name": sentiment.get("place_name") or details.get("name"),
            "address": details.get("address"),
            "rating": details.get("rating"),
            "user_ratings_total": details.get("user_ratings_total"),
            # Prefer photo from sentiment/cache; fall back to details helper
            "photo_url": sentiment.get("photo_url") or details.get("photo_url"),
        }
