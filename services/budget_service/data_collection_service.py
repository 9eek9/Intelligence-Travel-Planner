...
class DataCollectionService:
    ...
    @staticmethod
    def save_prediction(db: Session, user_id, input_features, prediction, method='ml'):
        """
    ...
    @staticmethod
    def update_actual_spending(db: Session, feedback_id, actual_spending):
        """
        ...
        """
        try:
            feedback = db.query(BudgetFeedback).filter(BudgetFeedback.id == feedback_id).first()
            if not feedback:
    ...
    @staticmethod
    def submit_user_feedback(db: Session, feedback_id, rating, was_helpful, comments=None):
        """
    ...
    @staticmethod
    def get_user_feedbacks(db: Session, user_id, limit=10):
        """Get recent feedbacks for a user"""
        return db.query(BudgetFeedback)\
            .filter(BudgetFeedback.user_id == user_id)\
    ...
    def get_feedback_stats(db: Session):
        ...
        total = db.query(BudgetFeedback).count()
        with_actual_spending = db.query(BudgetFeedback).filter(
            BudgetFeedback.actual_accommodation_amt.isnot(None)
        ).count()
        with_ratings = db.query(BudgetFeedback).filter(
            BudgetFeedback.user_rating.isnot(None)
        ).count()
        
        ml_predictions = db.query(BudgetFeedback).filter_by(prediction_method='ml').count()
        rule_predictions = db.query(BudgetFeedback).filter_by(prediction_method='rule_based').count()
    ...
    @staticmethod
    def export_for_retraining(db: Session, min_samples=100, output_path='ml_models/data/real_user_data.csv'):
        """
        ...
        feedbacks = db.query(BudgetFeedback).filter(
            BudgetFeedback.actual_accommodation_amt.isnot(None)
        ).all()
